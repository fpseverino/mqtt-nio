...
public final actor MQTTNewConnection: Sendable {
                    ...
                    do {
                        ...
                        if configuration.useSSL {
                            ...
                            try channel.pipeline.syncOperations.addHandler(
                                NIOSSLClientHandler(context: sslContext, serverHostname: configuration.sniServerName ?? host)
                            )
                        }
                        #endif
    ...
    private static func _getBootstrap(
        ...
        host: String,
        logger: Logger
    ) throws -> any NIOClientTCPBootstrapProtocol {
        ...
        if let tsBootstrap = NIOTSConnectionBootstrap(validatingGroup: eventLoopGroup) {
            ...
            if configuration.useSSL {
                return tsBootstrap.tlsOptions(options)
            }
            return tsBootstrap
        }
        #endif
